...
export default class HealthCheckHandler {
  constructor(
    private targetList: IHealthCheck[],
    private cascadeFailure: number = 0
  ) {}

  private static getCheckNameIndexInAccumulator = (
    target: IHealthCheck,
    accumulator: TargetHealthCheck[]
  ): number => {
    return accumulator.findIndex(
      (checkTarget) =>
        checkTarget.checkName === target.checkName &&
        checkTarget.target === target.target
    );
  };

  private static getTargetIndexInAccumulator(
    target: TargetHealthCheck,
    accumulator: Target[]
  ): number {
    return accumulator.findIndex(
      (checkTarget) => checkTarget.target === target.target
    );
  }

  private calcNewAggregatedStatus(
  ...
  private updateAggregatedTargetData(
    ...
    const failedChecksCount =
      currentTarget.status === "Failed"
        ? aggregatedTarget.failedChecks + 1
        : aggregatedTarget.failedChecks;
    const totalChecksCount = aggregatedTarget.totalChecks + 1;
    const aggregatedStatus = this.calcNewAggregatedStatus(
      failedChecksCount,
      totalChecksCount
    );
  ...
  private static updateAggregatedHealthCheckData(
    aggregatedHealthCheck: TargetHealthCheck,
    currentHealthCheck: IHealthCheck
  ) {
    const parsedCheckDate = moment(currentHealthCheck.completedAt);

    const updatedLastIssue =
      currentHealthCheck.status === "Failed" &&
  ...
  public groupByCheckName(): TargetHealthCheck[] {
    return this.targetList
      .reduce((targetAcc, currentTarget) => {
        const targetIndex = HealthCheckHandler.getCheckNameIndexInAccumulator(
          currentTarget,
          targetAcc
        );
        ...
        if (targetIndex === -1) {
          const aggregatedHealthCheck: TargetHealthCheck = {
            ...
            lastIssue:
              currentTarget.status === "Failed"
                ? moment(currentTarget.completedAt)
                : undefined,
          };
  ...
  public groupByTarget(): Target[] {
    ...
    return groupedByCheckName
      .reduce((targetAcc, currentTarget) => {
        const targetIndex = HealthCheckHandler.getTargetIndexInAccumulator(
          currentTarget,
          targetAcc
        );
