...
export default class ErrorsReportChecksStore {
  ...
  public async errorsReportChecksListGetAll() {
    const errorsReportChecksListJson =
      await this.fetchErrorsReportChecksListAsync();

    if (!errorsReportChecksListJson)
      throw new Error("errorsReportChecksList is undefined");

    // TODO remove map from there
    runInAction(() => {
      this.errorsReportChecksList = errorsReportChecksListJson.data.map(
        (errorReportDataElem) => {
          return {
            ...errorReportDataElem,
  ...
  private async fetchErrorsReportChecksListAsync(): Promise<HealthCheckResponse> {
    const errorsReportChecks = await fetch(
      `${this.apiUrl}lastFailedChecks/day/${this.day.format("YYYY-MM-DD")}`
    );
