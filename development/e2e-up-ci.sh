...
docker run -i -e CYPRESS_BASE_URL -e CYPRESS_VIDEO -e DISPLAY=:${XVFB_PORT} --entrypoint=bash -d --network host --name="${CYPRESS_ID}" cypress/included:8.5.0
...
docker exec -i -w /e2e "${CYPRESS_ID}" 'cypress' 'run' '--browser' 'chrome' '--headless' '--spec' 'cypress/integration/fury-application-status-scenario-2_spec.js'
...
docker exec -i -w /e2e "${CYPRESS_ID}" 'cypress' 'run' '--browser' 'chrome' '--headless' '--spec' 'cypress/integration/fury-application-status-scenario-3_spec.js'
